...
// Column set mirrored in 00298_sync_status_preserving_upserts.sql
...
export function transformRecipe(doc: MongoRecipe) {
  const createdAt = doc.createdAt instanceof Date
    ? doc.createdAt.toISOString()
    : doc.createdAt ? new Date(doc.createdAt).toISOString() : undefined;
  const updatedAt = doc.updatedAt instanceof Date
    ? doc.updatedAt.toISOString()
    : doc.updatedAt ? new Date(doc.updatedAt).toISOString() : undefined;
  ...
  return {
    ...
    target_attenuation: doc.targetAttenuation ?? null,
    status: "complete",
    is_active: true,
...
export function transformTransfer(doc: MongoTransfer) {
  return {
    ...
    from_vessel_id: null as string | null,
    to_vessel_id: null as string | null,
