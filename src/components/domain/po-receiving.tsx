...
export function POReceiving({
  ...
  const receiveMutation = useMutation({
    mutationFn: async (entries: ReceiveEntry[]) => {
      ...
      const { error } = await supabase.from("po_receives").insert(receivesToInsert);

      if (error) throw error;

      // Check if PO is fully received and update status
      const allFullyReceived = lineItems?.every((item) => {
        const receiving = receives[item.id]?.quantity || 0;
        return item.received_quantity + receiving >= item.quantity;
      });
