...
export function YeastLineageDisplay({ pitchId }: YeastLineageDisplayProps) {
  // Find the root of the lineage using the server-side recursive CTE (avoids N+1 queries)
  const { data: rootId, isLoading: rootLoading } = useQuery({
    queryKey: yeastKeys.lineageRoot(pitchId),
    queryFn: async () => {
      const supabase = createClient();
      const { data, error } = await dynamicRpc(supabase, "get_yeast_lineage_root", { p_pitch_id: pitchId });
      if (error) throw error;
      return (data as string) ?? pitchId;
    },
  });
