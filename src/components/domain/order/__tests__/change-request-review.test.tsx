...
import { changeRequestKeys, entityKeys } from "@/lib/query-keys";
...
describe("ChangeRequestReview approval", () => {
  it("refreshes every local order projection changed by approval", async () => {
    ...
    expect(invalidateSpy).toHaveBeenCalledWith({
      queryKey: changeRequestKeys.forOrder(ORDER_ID),
    });
    expect(invalidateSpy).toHaveBeenCalledWith({
      queryKey: changeRequestKeys.pendingForOrder(ORDER_ID),
    });
    expect(invalidateSpy).toHaveBeenCalledWith({
      queryKey: entityKeys.all("orders"),
    });
    expect(invalidateSpy).toHaveBeenCalledWith({
      queryKey: entityKeys.all("order_list_details"),
    });
    expect(invalidateSpy).toHaveBeenCalledWith({
      queryKey: entityKeys.detail("order_items_for_review", ORDER_ID),
    });
