...
describe("POST /api/chat/write", () => {
  it("inserts a confirmed reading as a batch_logs measurement, re-stamping the timestamp", async () => {
    ...
    expect(insertArgs.batch_id).toBe(BATCH_ID);
    expect(insertArgs.log_type).toBe("measurement");
    expect(insertArgs.data).toMatchObject({
      reading_type: "gravity",
      value: 12.5,
      unit: "plato",
    });
    // The stale, client-proposed timestamp is never trusted — the route
    // re-stamps at execution time so a card confirmed hours later doesn't
    // backdate the reading.
    expect(insertArgs.data.timestamp).not.toBe("2026-08-11T12:00:00.000Z");
    expect(new Date(insertArgs.data.timestamp).toString()).not.toBe(
      "Invalid Date",
    );
  });
  ...
  it("converts to the canonical unit before range-checking (60°C is out of range, not 60°F)", async () => {
    const sb = makeSupabase({});
    fixture.supabase = sb.supabase;

    const response = await POST(
      request(
        readingBody({ reading_type: "temperature", value: 60, unit: "c" }),
      ),
    );

    // 60°C = 140°F, outside the 32-100°F range — must be rejected, not
    // treated as a valid 60°F reading.
    expect(response.status).toBe(422);
    expect(sb.fromSpy).not.toHaveBeenCalled();
  });

  it("accepts a Celsius value that converts within the canonical °F range", async () => {
    const sb = makeSupabase({
      batches: [
        {
          data: { id: BATCH_ID, batch_code: "B-042", status: "fermenting" },
    ...
    // 18°C = 64.4°F, well within the 32-100°F range.
    const response = await POST(
      request(
        readingBody({ reading_type: "temperature", value: 18, unit: "c" }),
      ),
    );
  ...
  it("rejects a unit the reading type does not support, even for an in-range value", async () => {
    const sb = makeSupabase({});
    fixture.supabase = sb.supabase;

    const response = await POST(request(readingBody({ unit: "psi" })));
