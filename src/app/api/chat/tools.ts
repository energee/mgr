  ...
  return {
    ...
    recordBatchReading: tool({
      ...
      execute: async ({
        ...
        // Pass resolvedUnit so range checks compare against the canonical
        // unit (e.g. a Celsius value is converted to °F before being checked
        // against the °F-denominated min/max) instead of the raw value.
        const validation = validateReading(type, value, resolvedUnit);
        if (!validation.valid) {
          throw new Error(
