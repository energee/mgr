...
| `water_addition_profiles` | `water_addition_profiles_select` | Catalog reference data; companion `_write` gates writes by `settings:manage` (00197, guarded). NOTE: verified 2026-07-05 the table has been dropped from live out-of-band — the guard no-ops everywhere; finding CRITICAL-1 is moot. The customer-portal tables (`customer_portal_users`, `order_change_requests`, `order_change_request_items`) were likewise dropped from live; 00197's staff-tightening applies only on migrations-built databases. |
| `entity_revisions` | `entity_revisions_select` | Immutable audit log. **Corrected 2026-07-25 (#604):** the previous justification — "rows reference RLS-protected domain rows, so the audit row alone does not leak" — was false; `old_data`/`new_data` hold full `to_jsonb()` row copies, so the open `auth.uid() IS NOT NULL` read exposed every order, recipe, purchase order and pricing row to portal customers. 00275 replaces it with a `CASE entity_type` mapping to the tracked table's own read permission, defaulting unenumerated types to `settings:manage`. No longer a permissive policy. Inserts constrained by `changed_by = auth.uid()` in 00102. |
...
| `00275_entity_revisions_permission_gated_select.sql` | Replaces the open `entity_revisions_select` with a `CASE entity_type` mapping to each tracked table's read permission; unenumerated types fail closed to `settings:manage`. Corrects the false 00198 RLS-EXCEPTION comment (#604). |
