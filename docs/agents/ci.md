...
| `codegraph-refresh.yml` | Push to main touching graph inputs (`src/**`, migrations, snapshot, `docs/feature_list.json`, agent/spec/data-model docs, `tools/codegraph/**`) | Rebuilds `tools/codegraph/graph.json` (deterministic passes only, ~8s; LLM edges carried over) and lands it through an auto-merged bot PR, same shape as `progress.yml` — the committed graph must never drift silently again (#796). Separately, the required `Static Checks` job runs `tools/codegraph/doctor.ts --strict` against a fresh rebuild of the PR's own tree: new stale-cache / multi-write / dead-db / untested-write findings fail the PR unless deliberately baselined into `tools/codegraph/doctor-allowlist.json`. |
