...
| Docs Dream Sweep (`docs-dream.yml`) | cron `0 4 * * 6` (Sat 04:00 UTC) + dispatch | Deterministic broken-link report (`.github/scripts/check-doc-links.ts`) first, then a bounded agent fixes broken links and provable contradictions across AGENTS.md/CLAUDE.md/README.md/docs (≤15 fixes/run) into ONE docs-only `docs-dream` PR; ambiguous conflicts are listed in the PR body, never guessed. Never edits `docs/feature_list.json`, `PROGRESS.md`, or `docs/progress/**` | `docs-dream` PR (docs only), consumed by the human reviewing it | Disable workflow; close its PRs. Docs-only, so nothing else depends on it |
...
| `prune-log-tables` (migration 00296) | daily 05:10 UTC | `prune_log_tables()` deletes rows older than 90 days from `square_sync_log`, `qbo_sync_log`, `mongodb_sync_log`, `slack_notification_log`, `email_notification_log`, and older than 365 days from `entity_revisions`; returns per-table deleted counts | trimmed log tables (row counts in the `cron.job_run_details` result) | `SELECT cron.unschedule('prune-log-tables');` on live — tables just resume growing |
| `cleanup-old-notifications` (migration 00296; function from 00020) | daily 05:15 UTC | `cleanup_old_notifications(90)` deletes expired notifications and read notifications older than 90 days | trimmed `notifications` table | `SELECT cron.unschedule('cleanup-old-notifications');` on live |

All five jobs tolerate environments without `pg_cron` (CI replays) — the
