...
| Tests (vitest + Playwright)                   | B− | 2026-07-01 | 1920 vitest passing across 109 files; domain/services/contexts characterized (2026-07-01 loop). Coverage gate: per-directory floors in `vitest.config.ts` (lines/functions — lib 50/38, domain 85/88, services 85/90, contexts 62/60), enforced in CI (`test.yml` runs `test:coverage`) and `make check-coverage`; ratchet floors up as coverage grows. Shared test infra: `src/test/supabase-mock.ts`, `src/test/react-harness.ts`; suite TZ pinned to America/New_York. Hooks and entity configs still lack dedicated tests. E2E smoke covers 5 flows; deeper flows are `test.skip` pending seed data. |
