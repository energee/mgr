...
## Milestone-C continuation prompt (A0/A/B SHIPPED — paste into a fresh session to resume at C)

> Resume the Square POS bin-sync feature at **Milestone C**. Branch **`feat/square-pos-bin-sync`**, plan **`docs/plans/2026-07-07-square-pos-bin-sync.md`**, tracking **PR #360**, status in **`PROGRESS.md`** (read the top `2026-07-08` Phase-2 entry). Read the plan (esp. Milestones C + D) and that PROGRESS entry first.
>
> **DONE — live + committed + pushed through `656da25e` (do NOT redo):**
> - **PR #344 merged to main** (`c3dba263`) — `00207` `keg_filled_contents` + netted `keg_inventory` are on main + this branch.
> - **A0 `00219_bin_placement.sql`** — `packaging_sessions.default_bin_id` + `bins.is_default_fg` (partial-unique per location); `AFTER INSERT ON finished_goods` trigger `place_finished_good_in_bin()` (SECURITY DEFINER, non-keg only, resolves session `default_bin_id` → primary-location `is_default_fg` bin → NOTICE+skip, never blocks); `revise_packaging_session` mirrors FG-qty onto `bin_inventory`. Entity UI: bin `is_default_fg` toggle + packaging-session `default_bin_id` picker.
> - **A `00220_keg_bin_tracking.sql`** — `keg_transactions.from_bin_id`/`to_bin_id` (FK bins, ON DELETE SET NULL, nullable); `record_keg_transaction` is now a **single 16-arg overload** (appended `p_from_bin_id`/`p_to_bin_id`, old 14-arg DROPped); `keg_inventory`/`keg_inventory_with_details`/`keg_filled_contents` carry `bin_id` parallel to location. Keg-transaction UI: From/To Bin selectors.
> - **B `00221_sellable_inventory_view.sql`** — `sellable_inventory` view (`security_invoker`): packaged FGs in bins (`bin_inventory ⋈ fg ⋈ selling_formats ⋈ containers WHERE type <> 'keg' AND qty>0`, `source='packaged'`) `UNION ALL` `keg_filled_contents` (`source='keg'`). Shape: **`bin_id, location_id, brand_id, selling_format_id, finished_good_id, quantity, source`**. The `<> 'keg'` filter is the whole double-count guard.
> - **Migration head on live = `00221`; next new migration = `00222` (C).** All three A0/A/B verified on live via self-rolling-back blocks (+ an MCP rolling-back netting check for A).
>
> **Locked decisions (do NOT re-litigate):** D3 oversell = **clamp-to-zero + flag**; A6 = NULL/no backfill; the bin-resolution chain above; `sellable_inventory` shape above.
>
> **Schema/tooling gotchas discovered this session (rely on these):**
> - `finished_goods` / `packaging_sessions` / `session_line_items` carry **NO location** — the A0 fallback keys off the **primary location** (`locations.is_primary`), not the FG's.
> - `record_keg_transaction` is a **single 16-arg function** now (bins appended, defaults NULL). Callers passing the original args still work.
> - `customers.customer_type` is **NOT NULL**; ship keg_transactions require `customer_id` (`valid_ship_transaction` CHECK). Keg FGs are skipped by the A0 placement trigger (never in `bin_inventory`).
> - **`00210`** (vessel_transfer_integrity, PR #348) was `migration repair --status reverted` on live (it was applied out-of-band with no local file); #348 must re-apply it idempotently when it merges. #348 is independent of this work.
> - **`.env` is tracked in git**; the working read-only DB URL lives in **gitignored `.env.local`** (it must be quoted — the password contains `#` — and %-encoded). Consider `git rm --cached .env`.
> - **Apply migrations:** `printf 'y\n' | supabase db push --include-all` (CLI has saved creds; run with the command sandbox disabled — the pooler host is blocked). **Verify on live** with the Supabase MCP via a `DO $$ … RAISE EXCEPTION 'RESULT …' … $$;` block that rolls itself back. **Drift snapshot** (needed only when a migration changes a FUNCTION/TRIGGER/base TABLE — **views are NOT captured**): `PATH="/opt/homebrew/opt/libpq/bin:$PATH" ./node_modules/.bin/dotenv -e .env.local -- bash scripts/check-live-drift.sh --update` then re-run without `--update` to confirm `OK`.
> - A **GateGuard** hook fact-forces before the first Bash + every Edit/Write + destructive Bash — present the required facts inline, then retry the same call.
>
> **Milestone C — build in this order (per plan C1–C7):**
> 1. **C1 `00222_square_bin_pos_config.sql`** (data-layer-expert): add `bins.square_location_id text` + `bins.pos_sales_channel_id uuid REFERENCES sales_channels(id)`; create `square_locations` (`square_location_id text PRIMARY KEY, name text, status text, synced_at timestamptz`); **data-migrate** existing `locations.(square_location_id, pos_bin_id)` → set those two columns on the referenced `pos_bin` bin, **then `DROP` `locations.square_location_id` + `locations.pos_bin_id`** (destructive — migrate first, in the same migration). RLS: `square_locations` readable by staff, writable by `integrations:manage`. Self-verifying DO block. **This changes base tables → regenerate the drift snapshot after.**
> 2. **C2** regen types.
> 3. **C4** (integrations-expert, behavior-preserving — do early/in parallel): `resolveTaproomPrices(brandIds)` → `resolveChannelPrices(brandIds, salesChannelId)` in `src/integrations/square/pricing.ts` (drop the `code='taproom'` lookup; take the channel id from the caller). Update all callers. Parity test: passing the taproom channel id reproduces today's prices byte-for-byte.
> 4. **C3** (integrations-expert): `listSquareLocations()` in `src/integrations/square/client.ts` (Square `locations.list`); route `POST /api/square/locations/refresh` (`withPermission("integrations:manage")`) upserting `square_locations`. Unit test with a mocked Square client.
> 5. **C5** (integrations-expert): rewrite `src/app/api/square/sync/catalog/route.ts` + `sync/inventory/route.ts` to iterate **POS-configured bins** (`bins` with BOTH `square_location_id` and `pos_sales_channel_id` set), read stock from **`sellable_inventory`** (single query, filtered by `bin_id`), price via `resolveChannelPrices(brandIds, bin.pos_sales_channel_id)`, push to `bin.square_location_id`. `square_sync_log` keyed by bin. Remove the old two-query bin+keg reconciliation.
> 6. **C6** (entity-architect + ui-systems-expert): bin entity gets a `square_location_id` picker (relation to `square_locations`) + `pos_sales_channel_id` (relation to `sales-channel`); `src/app/(app)/settings/integrations/page.tsx` gets a "Refresh Square locations" action + a per-bin POS-config surface.
> 7. **C7** confirm `square_catalog_map` needs no location dimension for v1 (single price); document.
>
> **Then Milestone D** (integrations-expert + data-layer-expert): in `src/app/api/square/webhook/route.ts`, resolve the Square location → its POS bin (`bins.square_location_id`), pick the FG **within that bin** (FIFO over `bin_inventory ⋈ fg`), and **explicitly decrement `bin_inventory`** for the resolved bin (D0/D2) with **clamp-to-zero + flag** on oversell (D3). Preserve the existing signature-verify + replay-window + `event_id` dedup. Keg pours (D4) deplete the tapped filled keg's remaining volume. Keep the `taproom_sale` allocation as the audit/TTB ledger.
>
> **Repo rules:** `bun lint && bun typecheck && bun test` before every commit; regenerate `src/types/supabase.ts` after each migration; regenerate + verify the live-drift snapshot after each migration that touches a function/trigger/base-table lands live; expert agents per `CLAUDE.md` (C is integrations-heavy — use `integrations-expert`); **NEVER Co-Authored-By**. Commit + push each milestone to PR #360 and confirm cost checkpoints with the user (this session ran ~$243 through B).
