...
- [x] **16. Vessel integrity** (M3, M4, M5) — owner: `data-layer-expert` — **PR (branch `fix-audit-vessel-integrity`), 00210 applied live**
  ...
  DONE 2026-07-07 (decisions: idempotency key / empties_source column). M3: `vessel_transfers.idempotency_key` UUID + partial unique index; dialog mints one key per open (dedups double-click → 23505). M4: `handle_vessel_transfer()` rejects a transfer whose destination already holds a different batch (proven via rolled-back test). M5: `empties_source` boolean (default true); trigger frees the source only on a full move; dialog sets it `volume_bbl >= remaining`. Live impact zero (schema/trigger only).
