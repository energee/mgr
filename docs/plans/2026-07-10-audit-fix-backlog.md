...
- [x] **2. Close the staff OTP self-registration door** (DL-2, H) — owner: `data-layer-expert` — **PR #368**
  `shouldCreateUser: false` on `src/app/(auth)/login/login-form.tsx` passwordless path; shared `otpSignInErrorMessage()` mapper applied to staff AND portal forms; new login-form test file. **Residual user action DONE 2026-08-22: the hosted "allow new signups" toggle is confirmed off** (flagged since 2026-07-06; safe because no in-app path depends on it). Verified in the dashboard — the setting has no repo representation, so CI cannot catch a future re-enable.
...
- [x] **21. Integration-harness fixtures + plpgsql guard tests** (TC-1 C, TC-2, TC-7, TC-10) — owner: `test-surgeon` + `data-layer-expert` — **branch `test/plpgsql-guard-fixtures`; see `docs/progress/2026-08-21-plpgsql-guard-tests.md`**
  **Audit premise corrected:** TC-1's "no usable fixtures" CRITICAL framing was stale — `_helpers/role-client.ts`, `_fixtures/seed-roles.sql`, `scripts/db-local.sh` and 36 files / 239 behavioral tests already existed. The real gap was guard coverage plus one reusable seed helper.
  DONE: `_helpers/packaging-fixture.ts` (reusable packaging-chain builder, package/keg container option, `completePackagingSession`); `debit_bin_inventory` + `credit_bin_inventory` clamp coverage (16 tests — exact-sellout-vs-oversell boundary, zero clamp vs `chk_bin_inventory_quantity_nonneg`, negative-qty refusal, missing-row path); keg receive→fill→ship netting (9 tests — the received = empty+filled+shipped conservation invariant that catches the `HAVING sum > 0` inflation behind 00228/00229/00232/00234, plus the 00238 owner re-attribution and its deposit half); `revise_packaging_session` (+16 tests — every input-validation refusal with rows-unchanged assertions, revalue up/down, audit note, re-revision, and the `revised` status flip). Integration suite 239 → 280 tests, all green. Availability/outbound guards (00212/00216) needed nothing — already covered by `inventory-guards.test.ts`.
  **Found and filed: issue #917** — `enum_values` registry drift makes `receive`/`ship`/`clean`/`retire`/`maintain` unrepresentable on a chain-replayed DB, so a keg fleet cannot be received and `create_keg_ship_transactions_from_order` cannot run locally. `keg-netting.test.ts` seeds the missing rows in-transaction (`seedEnumRegistry`); **delete that shim when #917 lands**.
