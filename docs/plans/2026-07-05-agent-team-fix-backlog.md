...
| 7 | **DONE** (PR #338, `0d6ceb41`) — **Transition side-effects bypass**: `api/batches/[id]/transfer/route.ts:41-47` updates `batches.status` directly, never calling `runTransitionSideEffects` — the exact 2-of-4-paths silent-skip class `src/services/transition-side-effects.ts` was built to close. Verify whether a registered `(batches, toState)` effect applies to transfer target states (if yes, live silent-skip bug); either way route the update through the registry, and add an enforcement test that walks transition call sites (mirroring `entity-configs.test.ts`'s route-tree walk) so the next bypass fails CI. | `src/app/api/batches/[id]/transfer/route.ts`, `src/services/transition-side-effects.ts` | ~0 (correctness, not LOC) | Med | No | PR #336 review — **bug-fix track** |
...
| #7 transition side-effects bypass | [#338](https://github.com/energee/mgr/pull/338) `0d6ceb41` | +110 (route fix + enforcement test) | BUG FIX: API `packaging → completed` skipped `completeBatchConsumption`; red test first, then registry call wired in. `transition-call-sites.test.ts` now walks every `src/app/api/**/route.ts` for bypasses (suite 1,957 → 1,959) |
