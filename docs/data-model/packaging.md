...
| planned_quantity | NUMERIC | Planned quantity (NUMERIC since 00288 — MongoDB-era sessions carry fractional quantities like 70.5 cases) |
| actual_quantity | NUMERIC | Actual quantity (NUMERIC since 00288) |
| created_at | TIMESTAMPTZ | Created timestamp |
...
Dropped by `00294_drop_dead_schema_objects.sql` (schema audit 2026-08-21):

- `packages` (00001) — superseded by `finished_goods` + `selling_formats`;
  zero readers/writers remained.
- `pricing_channel_formats` (predecessor of [`channel_formats`](#channel_formats),
  re-captured in 00285 which already labeled it superseded) — retirement scoped
  in issue #724; executed by 00294.
