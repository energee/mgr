...
### Confirm-Gated Write Tools (Phase 4C, extended in 4B)

**Every write tool goes through the confirmation gate. There is no navigation-and-prefill write path any more** — Phase 4B converted the last of them, and no tool returns `action: "navigate"`.

A write tool persists nothing. It returns a `ConfirmWriteIntent` (`action: "confirm_write"`); the chat panel renders it as a Confirm/Cancel card; on confirm the client POSTs the pending payload to `POST /api/chat/write`. That route re-validates the payload against `chatWriteRequestSchema` (`src/lib/schemas/chat-write.ts`) and executes it with the caller's session Supabase client, so RLS remains the authority. The route is wrapped in `withAuth` and resolves a **per-action** permission through `CHAT_WRITE_PERMISSIONS` — not one blanket permission — so a future write against another domain cannot inherit batch rights.

`chatWriteRequestSchema` is a discriminated union with one explicit member per write, deliberately **not** a generic `{table, payload}` passthrough: the union is the trust boundary, so a model can only ever propose a shape that was designed and permission-mapped in advance.

| Tool | Description | Input |
|------|-------------|-------|
| `recordBatchReading` | Record a fermentation reading on an active batch (saved to `batch_logs` as a `measurement`) | `batchId?`, `batchNumber?`, `readingType`, `value`, `unit?`, `notes?` |
| `transitionBatch` | Move a batch to a new state, through `entityService.transition` (the atomic RPC that owns side effects) | `batchId?`, `batchNumber?`, `toState: enum` |
| `createPackagingSession` | Create a packaging session, re-validated through the entity's own form schema | `sessionDate`, `notes?` |
| `createBatch` | Create a batch from a recipe (`batch_code` is generated by a DB trigger) | `recipeId`, `name`, `plannedStartDate?`, `volumeBbl?` |

`transitionBatch` accepts only `CHAT_TRANSITION_STATES` (`fermenting`, `conditioning`, `packaging`, `completed`). `cancelled` and `archived` are excluded because they are `requiresAction` states that must run their own RPCs (vessel release, TTB loss).

A write tool's own eligibility check is **advisory**: the route re-resolves the record and re-checks before writing, because a batch can move between the proposal and the confirmation.

### Current Limitations

- **Replay is not idempotent** — a confirmation whose HTTP response is lost leaves the card in an `unknown` state that refuses to re-offer Confirm, but a panel remount still re-arms it. A replayed `create_batch` / `create_packaging_session` creates a real duplicate record. The durable fix is to thread the AI SDK `toolCallId` into the payload and dedupe server-side.
