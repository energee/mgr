...
- **NEXT — Square POS bin-sync feature (Phase-1 plan ready, awaiting approval).** Branch **`feat/square-pos-bin-sync`**, plan **[`docs/plans/2026-07-07-square-pos-bin-sync.md`](docs/plans/2026-07-07-square-pos-bin-sync.md)** (Milestones A–D + a paste-in continuation prompt). Config moves to the **bin** (bin↔Square-location 1:1, selectable sales channel, no hardcoded taproom, single-price-now); keg fleet gains a **bin dimension** (empties included); a **`sellable_inventory`** view unifies packaged FG + filled-keg contents; **Milestone D** debits bins from inbound Square sales. **Start with read-only spikes B0** (does filling a keg deplete the source FG/`bin_inventory`?) **+ D0** (is `bin_inventory` decremented by `taproom_sale` allocations today?). **Hard dep: PR #344 must merge first** (creates `keg_filled_contents` / rewrites `keg_inventory`, which Milestone A extends).
