...
- **NEXT — merge & reconcile.** The 2026-07-06 audit backlog is code-complete (only STOP/product items #10, #17, #19, #20 remain). The immediate follow-up is to merge the migration batch (#343–#359) as a group and regenerate main's live-drift snapshot. **Ready-to-run steps: [`docs/plans/2026-07-07-merge-and-reconcile.md`](docs/plans/2026-07-07-merge-and-reconcile.md).** Migrations `00206`–`00218` are already applied live; main's snapshot is at `00205`, so the live-drift CI is RED by design until reconciled. Merging is a user decision.
