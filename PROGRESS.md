...
- [x] **SENTRY-7479939863 — MGR-8: Error: No QueryClient set, use QueryClientProvider to set one** (sentry-fix/SENTRY-7479939863)
  Root cause: `NotificationsProvider` calls `useQueryClient()` which requires a `QueryClientProvider` ancestor. The `QueryClientProvider` was only in the root layout's `Providers` component, but Next.js App Router's async server component layout (`(app)/layout.tsx`) combined with Turbopack's dev-mode rendering can process the client subtree (`AppProviders` → `NotificationsProvider`) before the outer `Providers` context is established. Fix: added a `QueryClientProvider` directly inside `AppProviders`, co-locating it with its consumers so the provider chain is self-contained regardless of outer rendering order. The duplicate `QueryClient` config was extracted to `src/lib/query-client.ts` (`createAppQueryClient`) and reused by both `AppProviders` and the root `Providers`. Regression test at `src/contexts/__tests__/notifications.test.tsx` uses source-text inspection to assert that `AppProviders` always wraps `NotificationsProvider` in `QueryClientProvider`. All 1106 tests pass; lint and typecheck clean.
