...
-- Surface version in recipes_with_estimates so the editor can read it via
-- the existing select * query without a second round trip.
...
COMMENT ON VIEW recipes_with_estimates IS 'Recipes with style name, optimistic-lock version, and calculated OG/FG/ABV/IBU/SRM estimates.';
