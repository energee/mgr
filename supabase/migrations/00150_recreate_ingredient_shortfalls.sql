-- Migration: Recreate calculate_ingredient_shortfalls as safety net
--
-- Migration 00145 used DROP VIEW IF EXISTS recipe_ingredients_normalized CASCADE
-- which could potentially drop dependent database objects. While PostgreSQL
-- CASCADE on views does not drop functions (only dependent views, rules, and
-- triggers), this migration ensures calculate_ingredient_shortfalls exists
-- with the correct definition from 00138.
--
-- Uses CREATE OR REPLACE so this is safe to run regardless of current state.
...
COMMENT ON FUNCTION calculate_ingredient_shortfalls(INTEGER) IS
  ...
  'Recreated as safety net after DROP VIEW CASCADE in 00145.';
