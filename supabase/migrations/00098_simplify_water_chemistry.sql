-- Simplify Water Chemistry: Remove water_addition_profiles
-- Recipes now reference two water_profiles (source + target) directly.
-- Salt additions are calculated on-the-fly from the delta.

-- =============================================================================
-- 1. Add target_water_profile_id FK to recipes
-- =============================================================================

ALTER TABLE recipes
  ADD COLUMN target_water_profile_id UUID
    REFERENCES water_profiles(id) ON DELETE SET NULL;

COMMENT ON COLUMN recipes.target_water_profile_id IS 'Target water chemistry profile for calculating salt additions';

CREATE INDEX idx_recipes_target_water_profile
  ON recipes(target_water_profile_id)
  WHERE target_water_profile_id IS NOT NULL;

-- =============================================================================
-- 2. Drop recipes_with_estimates view (depends on water_addition_profile_id)
-- =============================================================================

DROP VIEW IF EXISTS recipes_with_estimates;

-- =============================================================================
-- 3. Clean up recipe_additions: remove profile-owned rows, drop profile_id
-- =============================================================================

DELETE FROM recipe_additions WHERE profile_id IS NOT NULL;

ALTER TABLE recipe_additions DROP CONSTRAINT IF EXISTS recipe_additions_owner_check;

ALTER TABLE recipe_additions DROP COLUMN IF EXISTS profile_id;

ALTER TABLE recipe_additions ALTER COLUMN recipe_id SET NOT NULL;

-- =============================================================================
-- 4. Drop water_addition_profile_id from recipes
-- =============================================================================

ALTER TABLE recipes DROP COLUMN IF EXISTS water_addition_profile_id;

-- =============================================================================
-- 5. Drop water_addition_profiles table
-- =============================================================================

DROP TABLE IF EXISTS water_addition_profiles CASCADE;

-- =============================================================================
-- 6. Recreate recipes_with_estimates view with target_water_profile_id
-- =============================================================================
...
-- =============================================================================
-- 7. Clean up _schema_registry
-- =============================================================================

DELETE FROM _schema_registry WHERE table_name = 'water_addition_profiles';

-- Update recipes registry: add target_water_profile_id to belongs_to
UPDATE _schema_registry
SET relationships = jsonb_set(
  relationships,
  '{belongs_to}',
  COALESCE(relationships->'belongs_to', '[]'::jsonb) || '[{"name": "target_water_profile", "table": "water_profiles"}]'::jsonb
)
