...
The sentinel exists because a blanket `[]` backfill is a claim, not an audit, and a false claim a check blesses is worse than an unread one. **The gate's coverage is partial and it says so on every run** — as of 2026-07-31, 24 entries are migration-backed (23 live — 22 snapshot-verified, 1 not verifiable from the snapshot — and 1 pending), 38 are audited as needing no schema change, and **none** read `"unaudited"` (the backlog reached 0 when #724 captured the last live-only table; #696). Do not trust that split from this sentence; run `make check-deploy-state`, which prints the current one. The unaudited count is ratcheted against `deployment_conventions.unaudited_backlog.count`, so adding to it takes a visible edit.
