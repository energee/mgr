...
The sentinel exists because a blanket `[]` backfill is a claim, not an audit, and a false claim a check blesses is worse than an unread one. **The gate's coverage is partial and it says so on every run** — as of 2026-08-12, 24 entries are migration-backed (24 live — 23 snapshot-verified, 1 not verifiable from the snapshot — and 0 pending), 38 are audited as needing no schema change, and **none** read `"unaudited"` (the backlog reached 0 when #724 captured the last live-only table; #696). Do not trust that split from this sentence; run `make check-deploy-state`, which prints the current one. The unaudited count is ratcheted against `deployment_conventions.unaudited_backlog.count`, so adding to it takes a visible edit.
...
- Never pick the next migration number from this checkout alone. Take the max across worktrees, remote branches, and `supabase migration list` (see [`gotchas.md`](docs/agents/gotchas.md)). `ls supabase/migrations/ | tail -1` only knows this tree and has caused three prefix collisions (#213, #545, #920).
...
2. **Run quality gates** (if code changed) — `make check`, or `make check-all` if cross-component.
   - Before opening a PR, also run `/simplify` first, then `/code-review` (review last, so it sees the post-simplification diff), and apply/triage the findings. A PreToolUse hook (`scripts/hooks/pr-review-gate.sh`) blocks `gh pr create` until you confirm this with `touch $(git rev-parse --git-dir)/pr-review-ok` (one-shot, consumed per PR; the git-dir is resolved from the session's cwd, so a worktree session touches its own). Run that `touch` as its **own** command — `touch … && gh pr create …` is always denied, because the hook inspects the whole command string before any of it runs (#786; [`gotchas.md`](docs/agents/gotchas.md)).
