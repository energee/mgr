...
2. **Status committed before side effects** (#434 entity transitions, #416 PO receiving): the terminal state lands, then inventory/fulfillment/accounting effects run fire-and-forget. A failed request or closed tab strands a state the UI cannot explain.
...
4. **Claim/replay units smaller than the side-effect set** (#443, #477 Square refunds): "exactly-once" claims that cover only part of the work. #477's reversal math read the *original* sale allocation instead of what remained after prior refunds — proportional/compensating math must read current state, not the initial snapshot.
...
8. **Derived data not recalculated by every mutation path** (#489 shipping materials): the interactive editor recalculates `order_materials`; the new atomic approval RPC applies the same line mutations and doesn't. Derivations must live behind one reusable entry point that every mutation path invokes — same lesson as `transition-side-effects.ts`.
...
- `src/services/transition-side-effects.ts` (the registry pattern derived work should follow)
- `src/lib/supabase/optimistic-lock.ts`
- `src/app/api/square/webhook/route.ts` (claim/dedup reference implementation)
