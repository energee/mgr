...
- **Child-row replacement must be atomic** (#446, #480, #488): editors that replace a parent's child rows (recipe malts/hops, additions, change-request items) via a DELETE request followed by an INSERT request lose data on mid-chain failure and interleave under concurrent edits. New editors must register with the shared atomic save boundary (the #446 recipe "Save All" RPC pattern), and deletes must be scoped to the exact subset being edited — #480's generic display path deleted recipe-wide while editing one category.
- **Derived data needs one shared entry point** (#489): the staff order-item editor recalculated `order_materials`, but the change-request approval RPC applying the same line mutations didn't — approved changes left stale shipping-material estimates. Any mutation path that changes rows feeding a derivation (order materials, allocations, balances) must call the same reusable server-side recalculation — the exact lesson `transition-side-effects.ts` encodes for state transitions. When adding a mutation path, enumerate its sibling paths and check what they recalculate.
- On any diff touching a multi-step write (transition + side effects, parent + child rows, entity + derived rows), dispatch `transaction-safety-reviewer` — 16 of 50 recent issues were this pattern (`docs/plans/2026-07-16-issue-insights.md`).
...
9. Child-row editors save through the shared atomic RPC boundary, never DELETE-then-INSERT via two PostgREST calls; deletes scoped to the edited subset (#446/#480/#488).
10. Mutation paths that touch derivation inputs invoke the shared recalculation entry point — verified against every sibling path, not just the new one (#489).
11. Multi-step write diffs get a `transaction-safety-reviewer` pass before merge.
