...
9. **Never ack an external event or report sync success before local persistence is verified** (#445, #443): QuickBooks accepted Bills/Invoices while the mapping upsert's `{error}` was silently discarded — remote and local permanently disagreed. Every awaited Supabase write in a sync path has its `error` checked, and the remote-write→local-persist ordering leaves a retryable record if the local half fails.
10. **Ingestion claims must cover the whole side-effect set in one transaction** (#443, #477): the Square claim covered dedup but allocation-insert and bin-debit were separate transactions, and refund reversal math read the *original* sale allocation instead of the post-refund remainder — sequential partial refunds under-reversed inventory. Compensating/proportional math reads current remaining state; claim + all side effects belong in a single RPC (00257 pattern).
11. **Destructive resyncs report partial failure truthfully** (#444): Mongo clean/resync ran unchecked bulk deletes then fallible rebuilds, recording thrown phases as `failed: 0`. Delete-then-rebuild needs a transactional boundary or accurate per-phase failure accounting — a success response after a partial failure is a data-loss amplifier on the next run.
12. On any diff touching a sync/webhook write chain, dispatch `transaction-safety-reviewer` (`docs/plans/2026-07-16-issue-insights.md` — this cluster produced 5 of the repo's recent severity:high issues).
