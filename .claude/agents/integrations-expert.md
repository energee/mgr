...
- Token persistence is one `save_qbo_tokens_atomic` RPC (00299) called from `token-manager.ts`: all four rows (`qbo_access_token`, `qbo_refresh_token`, `qbo_realm_id`, `qbo_token_expires_at`) commit in a single transaction serialized by an advisory xact lock, with the refresh-path compare-and-swap done DB-side. Disconnect deletes the token rows via `clear_qbo_tokens_atomic` under the same lock (config keys survive). The racy 00100 `save_qbo_tokens`/`clear_qbo_tokens` were dropped by 00299 (`save_qbo_client_credentials` is kept — it is the only credential writer). Don't add a direct `system_settings` write path beside the RPCs.
...
3. QBO token writes stay on the single `token-manager.ts` RPC path (`save_qbo_tokens_atomic` / `clear_qbo_tokens_atomic`, 00299) — no direct `system_settings` writes beside it; no third refresh implementation; single-flight guard preserved.
