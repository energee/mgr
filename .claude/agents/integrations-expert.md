...
- Token persistence is one `save_qbo_tokens_atomic` RPC (00297) called from `token-manager.ts`: all four rows (`qbo_access_token`, `qbo_refresh_token`, `qbo_realm_id`, `qbo_token_expires_at`) commit in a single transaction serialized by an advisory xact lock, with the refresh-path compare-and-swap done DB-side. Disconnect deletes the token rows via `clear_qbo_tokens_atomic` under the same lock (config keys survive). The racy 00100 `save_qbo_tokens`/`clear_qbo_tokens` were dropped by 00297 (`save_qbo_client_credentials` is kept — it is the only credential writer). Don't add a direct `system_settings` write path beside the RPCs.
...
3. QBO token writes stay on the single `token-manager.ts` RPC path (`save_qbo_tokens_atomic` / `clear_qbo_tokens_atomic`, 00297) — no direct `system_settings` writes beside it; no third refresh implementation; single-flight guard preserved.
...
5. Sync code fails loudly **before** a side effect commits, but stops over-throwing **after** it. Separate the primary write (external POST + its local mapping/row) from the closing audit write (`*_sync_log`/`mongodb_sync_log`): once the primary work has committed, a failed audit write must be **logged, not rethrown** as the operation's result — otherwise a succeeded sync reports failure to the operator and can halt dependent phases. Swallow-and-warn on the closing success-log write; throw only on pre-commit failures. Reference: QBO `completeSyncLogSuccess`/`recordSyncFailure` (`sync-utils.ts`), Mongo `completeSyncLog` (#474/#475). Still no swallowed catches on the primary path; per-item errors still land in the relevant `*_sync_log` table.
